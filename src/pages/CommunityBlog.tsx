...
export default function CommunityBlog() {
  ...
  const [newPost, setNewPost] = useState({ author_name: '', author_email: '', title: '', content: '' });
  const [newComment, setNewComment] = useState<Record<string, { name: string; email: string; text: string }>>({});
  ...
  const handleSubmitPost = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newPost.author_name || !newPost.author_email || !newPost.title || !newPost.content) {
  ...
  const handleSubmitComment = (postId: string) => {
    ...
    if (!commentData || !commentData.name || !commentData.email || !commentData.text) {
      alert('Please fill in all fields');
      return;
    }
    ...
    setNewComment(prev => ({ ...prev, [postId]: { name: '', email: '', text: '' } }));
  };
  ...
  return (
    <div className="blog-page community-blog">
        <div className="container section-padding">
          <h1 className="page-title fade-in">Community Blog</h1>
          <p className="page-subtitle fade-in">Share your thoughts and connect with the community</p>
                    ...
                    {comments[post.id] && comments[post.id].length > 0 && (
                      <div className="comments-list">
                        {comments[post.id].map(comment => (
                          <div
                            key={comment.id}
                            className={`comment ${comment.is_actor_response ? 'actor-comment' : ''}`}
                          >
                    ...
                    <div className="comment-form">
                      ...
                      <input
                        ...
                        onChange={(e) => setNewComment(prev => ({
                          ...prev,
                          [post.id]: { ...(prev[post.id] || { name: '', email: '', text: '' }), name: e.target.value }
                        }))}
                      />
                      <input
                        type="email"
                        ...
                        onChange={(e) => setNewComment(prev => ({
                          ...prev,
                          [post.id]: { ...(prev[post.id] || { name: '', email: '', text: '' }), email: e.target.value }
                        }))}
                      />
                      <textarea
                        ...
                        onChange={(e) => setNewComment(prev => ({
                          ...prev,
                          [post.id]: { ...(prev[post.id] || { name: '', email: '', text: '' }), text: e.target.value }
                        }))}
                      />
                      <button
                        className="btn-primary"
